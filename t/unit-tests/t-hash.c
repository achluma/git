...
static const char hash_file[] = "./t0013/shattered-1.pdf";

/* Return 1 if prereqs is satisfied, 0 otherwise */
static int check_prereq_SHA1_IS_SHA1DC()(void) {
...
static void check_hash_file(FILE *file, const char *expected, int algo) {
	if (!check_prereq_SHA1_IS_SHA1DC)
		test_skip("skipping sha1 collision tests, not using sha1collisiondetection");
		return;

	if (!check(!!file)) {
		test_msg("Error opening file %s", file);
		return;
	}

	check_hash_data(file, strlen(file), expected, GIT_HASH_SHA1);
}

#define TEST_SHA1_DC(file, expected) \
    	TEST(check_hash_file(file, strlen(file), expected, GIT_HASH_SHA1), \
        	"SHA1 (%s) works", #file)
...
int cmd_main(int argc, const char **argv) {
	...
	TEST_SHA1_DC(hash_file,"collision");
	TEST_SHA1_DC(hash_file,"38762cf7f55934b34d179ae6a4c80cadccbb7f0a");
