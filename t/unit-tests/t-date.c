...
static void check_show_date(const char *format, const char *TIME, const char *expected, const char *prereqs, const char *zone) {
	...
	tz = (strcmp(zone, "") == 0) ? atoi(arg) : atoi(zone);
...
static void check_approxidate(const char *given, const char *expected) {
	timestamp_t t;
	char expected_with_offset[128];
    	t = approxidate(given);
	
	snprintf(expected_with_offset, sizeof(expected_with_offset), "%s +0000", expected);
...
int cmd_main(int argc, const char **argv) {
	...
	// TEST(check_show_date("format-local:%Y-%m-%d %H:%M:%S", "1466000000 +0200", "2016-06-15 09:13:20", "", "EST5"), "Check_show_date - format-local:%%Y-%%m-%%d %%H:%%M:%%S works");
	TEST(check_show_date("format:%s", "123456789 +1234", "123456789", "", ""), "Check_show_date - format:%%s positive offset works");
	TEST(check_show_date("format:%s", "123456789 -1234", "123456789", "", ""), "Check_show_date - format:%%s negative offset works");
	TEST(check_show_date("format-local:%s", "123456789 -1234", "123456789", "", ""), "Check_show_date - format-local:%%s negative offset works");

	// # arbitrary time absurdly far in the future
	TEST(check_show_date("iso", "5758122296 -0400", "2152-06-19 18:24:56 -0400", "TIME_IS_64BIT,TIME_T_IS_64BIT", ""), "Check_show_date - iso works");
	TEST(check_show_date("iso-local", "5758122296 -0400", "2152-06-19 22:24:56 +0000", "TIME_IS_64BIT,TIME_T_IS_64BIT", ""), "Check_show_date - iso-local works");
	...
	// TEST(check_parse_date("2008-02-14 20:30:45", "2008-02-14 20:30:45 -0500", "EST5"), "check_parse_date - 2008-02-14 20:30:45 -0500 works");
