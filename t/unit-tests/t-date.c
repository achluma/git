...
static int check_prereqs(unsigned int prereqs) {
	bool is_time_64bit = (prereqs & TIME_IS_64BIT) != 0;
    	bool is_time_t_64bit = (prereqs & TIME_T_IS_64BIT) != 0;

    	if (is_time_64bit) {
        	if (sizeof(timestamp_t) != 8) {
           		return 0;
        	}
    	}
...
static void check_show_date(const char *format, const char *TIME, const char *expected, unsigned int prereqs, const char *zone) {
	...
	if (check_prereqs(prereqs) == 0){
		if ((prereqs & TIME_IS_64BIT) && (prereqs & TIME_T_IS_64BIT)) {
    			test_skip("missing prerequisites TIME_IS_64BIT and TIME_T_IS_64BIT");
		} else if (prereqs & TIME_IS_64BIT) {
    			test_skip("missing prerequisite TIME_IS_64BIT");
		} else if (prereqs & TIME_T_IS_64BIT) {
