...
static int check_is_64bit(const char *arg) {
    	if (!strcmp(arg, "TIME_IS_64BIT,TIME_T_IS_64BIT")) {
        	return (sizeof(timestamp_t) == 8 && sizeof(time_t) == 8) ? 0 : 1;
   	}
    	return -1;
}

static void check_show_date(const char *format, const char *TIME, const char *expected, const char *prereqs, const char *zone) {
	struct date_mode mode = DATE_MODE_INIT;
	char *arg;
	timestamp_t t;
	int tz;

	if (check_is_64bit(prereqs) == 1){
...
int cmd_main(int argc, const char **argv) {
	...
	TEST(check_show_date("iso8601", test_time, "2016-06-15 16:13:20 +0200", "", ""),
		"Check_show_date - iso8601 works");
	TEST(check_show_date("iso8601-strict", test_time, "2016-06-15T16:13:20+02:00", "", ""),
		"Check_show_date - iso8601-strict works");
	TEST(check_show_date("rfc2822", test_time, "Wed, 15 Jun 2016 16:13:20 +0200", "", ""),
		"Check_show_date - rfc2822 works");
	TEST(check_show_date("short", test_time, "2016-06-15", "", ""),
		"Check_show_date - short works");
	TEST(check_show_date("default", test_time, "Wed Jun 15 16:13:20 2016 +0200", "", ""),
		"Check_show_date - default works");
	TEST(check_show_date("raw", test_time, test_time, "", ""),
		"Check_show_date - raw works");
	TEST(check_show_date("unix", test_time, "1466000000", "", ""),
		"Check_show_date - unix works");
	TEST(check_show_date("iso-local", test_time, "2016-06-15 14:13:20 +0000", "", ""),
		"Check_show_date - iso-local works");
	TEST(check_show_date("raw-local", test_time, "1466000000 +0000", "", ""),
		"Check_show_date - raw-local works");
	TEST(check_show_date("unix-local", test_time, "1466000000", "", ""),
		"Check_show_date - unix-local works");

	TEST(check_show_date("format:%z", test_time, "+0200", "", ""),
		"Check_show_date - format:%%z works");
	TEST(check_show_date("format-local:%z", test_time, "+0000", "", ""),
		"Check_show_date - format-local:%%z works");
	TEST(check_show_date("format:%Z", test_time, "", "", ""),
		"Check_show_date - format:%%Z works");
	TEST(check_show_date("format-local:%Z", test_time, "UTC", "", ""),
		"Check_show_date - format-local:%%Z works");
	TEST(check_show_date("format:%%z", test_time, "%z", "", ""),
		"Check_show_date - format:%%z works");
	TEST(check_show_date("format-local:%%z", test_time, "%z", "", ""),
		"Check_show_date - format-local:%%z works");

	TEST(check_show_date("format:%Y-%m-%d %H:%M:%S", test_time, "2016-06-15 16:13:20", "", ""),
		"Check_show_date - format:%%Y-%%m-%%d %%H:%%M:%%S works");

	TEST(check_show_date("format-local:%Y-%m-%d %H:%M:%S", test_time, "2016-06-15 09:13:20", "", "EST5"),
		"Check_show_date - format-local:%%Y-%%m-%%d %%H:%%M:%%S works");

	TEST(check_show_date("format:%s", "123456789 +1234", "123456789", "", ""),
		"Check_show_date - format:%%s positive offset works");
	TEST(check_show_date("format:%s", "123456789 -1234", "123456789", "", ""),
		"Check_show_date - format:%%s negative offset works");
	TEST(check_show_date("format-local:%s", "123456789 -1234", "123456789", "", ""),
